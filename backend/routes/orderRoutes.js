...
router.post("/", protect, async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      const netPrice =
        item.quantity * product.price - item.quantity * totalScheme;
...
router.put("/:id/approve", protect, adminOnly, async (req, res) => {
  try {
    const order = await Order.findById(req.params.id).populate("retailer");
...
// Generate bill from approved order
router.post("/:id/generate-bill", protect, adminOnly, async (req, res) => {
  try {
    ...
    if (order.status !== "Approved") {
      return res
        .status(400)
        .json({ message: "Only approved orders can generate bills" });
    }

    if (order.billGenerated) {
      return res
        .status(400)
        .json({ message: "Bill already generated for this order" });
    }
    ...
    // Get collection day from retailer or use default
    const collectionDay = order.retailer.dayAssigned || "Monday";
    ...
    const bill = new Bill({
      ...
      dueAmount: order.totalOrderValue,
      status: "Unpaid",
      collectionDay,
      billDate: new Date(),
      assignedTo: order.retailer.assignedTo,
    ...
    order.generatedBillId = bill._id;
