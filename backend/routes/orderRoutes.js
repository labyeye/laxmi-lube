...
router.get("/export/excel", protect, adminOnly, async (req, res) => {
  try {
    ...
    const currencyColumns = ['price', 'scheme', 'otherScheme', 'totalScheme', 'netPrice', 'totalSale'];
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=orders_export_${new Date().toISOString().slice(0,10)}.xlsx`
    );
...
router.post("/", protect, async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      // Calculate values
      const netPrice = (item.quantity * product.price) - product.scheme + (item.otherScheme || 0);
      const totalSale = netPrice * item.quantity;
