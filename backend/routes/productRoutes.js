...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const nameCol = lowerHeaders.findIndex(
      ...
      const companyCol = lowerHeaders.findIndex(
      ...
      for (const [index, row] of jsonData.entries()) {
        ...
        try {
          ...
          const company =
            companyCol !== -1 ? row[companyCol]?.toString().trim() : "";
          ...
          if (existingProduct) {
            errors.push(
              `Row ${index + 1}: Product with code ${code} already exists`,
            );
            continue;
          }
